...
#define MAX_FILE 10
//Я так понимаю максимальное количество файлов 10
...
{
	//Зачем нам очередь?
	//(что бы помещать клиентов, которые пытаются получить доступ к одинаковому файлу
	//Если 2 клиента захотят писать в один файл, но как это тогда будет стыкова
	//ваться с кодом в main?
	//(в main работает событие, а помещаем в очередь в этом потоке)
	int index = (int)lp;
	...
	while (true)
	{
		...
		{
			if (masthr[index]->command == 'l')
			{
				//Удаляет первый элемент очереди
				//Почему удаляем именно первый элемент?
				//(скорее всего потомучто leave может придти только от первого 
				//элемента, т.к другим клиентам, которые пытались подключиться
				//мы ничего не отправляли и они ждут ответа(мои предположения)r
				q->pop();
				if (!q->empty()) {
					cout << "lea" << endl;
...
{	setlocale(LC_ALL, "Russian");
	...
	try
	{
		//Зачем заполнять элементы налами?
		...
		while(true)
		{
			...
			if (create)
			{
				...
				if (!findplace)
				{
					std::cout << "Cant create new thread" << endl;
					break;
				}
				...
				//Для чего нам параметр pos?
				//(чтобы в потоке обратится к соответсвующему элементу)
				a->Thread = CreateThread(NULL, NULL,&Dispath,(void*)pos, NULL, NULL);
				//Зачем здесь SetEvent?
				//(потому что есть новый клиент, он первый, поэтому переводим
