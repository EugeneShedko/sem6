...
HDFS OpenDFSFile(char* FileName, char* FileWay)
{
	if (EnterCA(FileName))
	{
		...
		if (ptrFile != INVALID_HANDLE_VALUE)
		{
			return ptrFile;
		}
		return 0;
...
bool LeaveCA(char* FileName)
{
	string str = "leave";
	str.append(FileName);
	//Что будет делать сервер, после отправки сообщения
	sendto(cS, str.c_str(), str.length(), NULL, (sockaddr*)&serv, len);
	return true;
}

//Почему bufsize именно 10?
int ReadDFSFile(HDFS hdfs, void* buf, int bufsize)
{
	DWORD s;
	if (ReadFile(hdfs, buf, bufsize, &s, NULL))
	{
		return s;
	}
	else return -1;
...
int WriteDFSFile(HDFS hdfs, void* buf, int bufsize)
{
	...
	if (WriteFile(hdfs, buf, bufsize, &s, NULL))
	{
		//Зачем нам сбрасывать буфер?
...
{	setlocale(LC_CTYPE, "Russian");
	...
	try
	{
		...
		{			
			...
			HDFS h;
			//получили HANDLE созданного файла
			if ((h = OpenDFSFile("Client", "\\\\Home-pc\\общий доступ в сеть\\text.txt")) == 0)
			{
				cout << "Ошибка открытия файла" << endl;
			}
			if (h)
			{
				//Почему прочитанный текст из файла не выводится на консоль?
				if(ReadDFSFile(h, buf, 10) > 0)
				strcat(buffer, " ");

				//Соединили время и ответ от сервера, но зачем?
				strcat(buffer, inBuf);
				cout << "Client: " << buffer << endl;

				if (WriteDFSFile(h, buffer, strlen(buffer)) == -1)
					cout << "Ошибка записи в файл" << endl;
				WriteDFSFile(h, "\r\n", strlen("\r\n"));
			}
			else break;
			//Не понимаю, почему, если файл не открылся выкидываем break?
			CloseDFSFile(h, "Client");
			Sleep(5000);
		}
